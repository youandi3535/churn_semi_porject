...
/*                   topic.js 안에
│
├── CHIP_META          → 각 차트 위 칩 텍스트 내용
├── TOPIC_COLORS       → 주제별 색상 (A=빨강, B=파랑 등)
├── TOPIC_BADGE        → 카드 우상단 뱃지 텍스트
├── TOPIC_DESC         → 주제 설명 텍스트
│
├── renderBarLine      → 막대+꺾은선 혼합
│     해당차트: A-01 상담 건수별 해지율
│     └── x.title.font.size:10
│     └── y.title.font.size:10  (해지율)
│     └── y2.title.font.size:10 (고객수)
│
├── renderLine         → 꺾은선
│     해당차트: A-02 100분당 상담건수 분위별 해지율
│               B-02 평균 요금 분위별 해지율
│               C-02 밤-낮 통화 차이 분위별 해지율
│               D-04 가입기간(로그) 분위별 해지율
│               E-03 음성사서함 횟수(로그) 분위별 해지율
│     └── x.title.font.size:10
│     └── y.title.font.size:10
│
├── renderHorizBar     → 가로막대
│     해당차트: A-03 통화 대비 상담비율 분위별 해지율
│     └── x.title.font.size:10  (X축이 해지율%)
│
├── renderCompareBar   → 비교막대 2개
│     해당차트: A-04 상담 상위 10% vs 일반 고객 해지율
│               C-04 낮 집중 사용 고객 vs 일반 해지율
│               D-03 고사용+장기 고객 vs 일반 해지율
│               E-02 음성사서함 사용 여부별 해지율
│     └── y.title.font.size:10  (X축엔 타이틀 없음)
│
├── renderBar          → 일반막대
│     해당차트: B-01 총 사용량 분위별 해지율
│               B-03 요금 변동성 분위별 해지율
│               C-01 밤 통화 비중 분위별 해지율
│               C-03 시간대 집중도 분위별 해지율
│               D-01 가입기간 분위별 해지율
│               E-04 총 통화횟수 분위별 해지율
│     └── x.title.font.size:10
│     └── y.title.font.size:10
│
├── renderClustered    → 교차분석 묶음막대 (Y축색 건드리지 않기로 함)
│     해당차트: B-04 사용량 × 평균요금 교차 해지율
│               E-01 평균요금 × 요금변동성 교차 해지율
│
├── renderMultiLine    → 교차분석 멀티라인 (Y축색 건드리지 않기로 함)
│     해당차트: D-02 사용량 × 가입기간 교차 해지율
│
├── drawCharts         → chart_type 보고 위 7종 중 하나 자동 실행
├── updateCardUI       → 버튼 클릭시 칩/뱃지/설명줄 교체
└── updateAI           → AI 분석 3종 챗봇 카드에 반영
=================================================================*/
    ...
    options: {
      ...
      scales:{
        x: { grid:{color:GRID_COLOR}, ticks:{color:TICK_COLOR},
             title:{display:true, text:data.x_label||'', color:TICK_COLOR, font:{size:13}} },
        y: { position:'left', min:0, max:Math.max(100,...data.values)+5,
             grid:{color:GRID_COLOR}, ticks:{color:TICK_COLOR, callback:v=>v+'%'},
             title:{display:true, text:'해지율 (%)', color:c, font:{size:11,weight:'700'}} },
    ...
    options:{
      ...
      scales:{
        x:{ grid:{color:GRID_COLOR}, ticks:{color:TICK_COLOR, callback:v=>v+'%'},
            title:{display:true, text:'해지율 (%)', color:c, font:{size:13}} },
  ...
  mountChart(canvasId, canvasId, {
    ...
    options:{
      ...
      scales:{
        x:{ grid:{color:GRID_COLOR}, ticks:{color:TICK_COLOR} },
        y:{ grid:{color:GRID_COLOR}, ticks:{color:TICK_COLOR, callback:v=>v+'%'},
            title:{display:true, text:'해지율 (%)', color:c, font:{size:13,weight:'700'}}, min:0 },
      },
    },
  });
}
    ...
    options:{
      ...
      scales:{
        x:{ grid:{color:GRID_COLOR}, ticks:{color:TICK_COLOR},
            title:{display:true, text:data.x_label||'', color:TICK_COLOR, font:{size:13}} },
        y:{ grid:{color:GRID_COLOR}, ticks:{color:TICK_COLOR, callback:v=>v+'%'},
            title:{display:true, text:'해지율 (%)', color:c, font:{size:13,weight:'700'}} },
      },
    },
  });
}
