...
/* =====================================================
  [D-2] autoInsight(data)  — 기존 유지 (가설 버튼용)
===================================================== */
async function autoInsight(data) {
  const box = document.getElementById('chatMessages');
  if (!box) return;
  box.innerHTML = '<span class="msg-loading">🤖 AI 종합 분석 중...</span>';
  try {
...
/* =====================================================
  [D-2b] loadAI(data)
  역할: 3개 챗봇 패널을 동시에(병렬로) 채우는 함수
  ★ 각 챗봇은 graph_data 원본을 독립적으로 분석
  ★ Promise.all → 3개 동시 요청 (순서 기다리지 않음 = 빠름)
===================================================== */
async function loadAI(data) {

  // 로딩 표시 먼저
  setText('ai1Text', '⏳ 분석 중...');
  setText('ai2Text', '⏳ 분석 중...');
  setText('ai3Text', '⏳ 분석 중...');

  // 3개 동시 요청 (병렬)
  const [r1, r2, r3] = await Promise.all([
    fetchAI('/ai_summary',  data),
    fetchAI('/ai_strategy', data),
    fetchAI('/ai_forecast', data),
  ]);
